...
/**
 * Cliente SOAP específico para a API SOC (UnidadeWs)
 * Estende o SoapService existente para customizações de segurança e métodos.
 */
class SocUnidadeClient {
  constructor() {
    this.wsdl = config.socUnidade.wsdl;
    this.username = config.socUnidade.username;
    this.password = config.socUnidade.password;
    this.client = null;
  }
  ...
  async createClient() {
    ...
    try {
      ...
      this.client = await soap.createClientAsync(this.wsdl, {
        wsdl_options: {
          timeout: config.socUnidade.timeout || 30000,
      ...
      throw new Error(`Erro ao criar cliente SOAP para SOC (Unidade): ${error.message}`);
    }
  }

  /**
   * Executa uma chamada SOAP para um método específico.
   * @param {string} method - Nome do método a ser chamado (ex: incluirUnidade)
  ...
  async call(method, params) {
    ...
    try {
      ...
      if (!client[method]) {
        ...
        throw new Error(`Método ${method} não encontrado no WSDL do SOC (Unidade)`);
      }
      ...
      throw new Error(`Erro na chamada SOAP para ${method} (Unidade): ${error.message}`);
    }
  }
  ...
  formatResponse(response) {
    ...
    // Tenta encontrar o retorno em diferentes caminhos possíveis
    const retorno = response?.UnidadeRetorno || response?.return || response;
    ...
    const resultado = {
      ...
      detalhes: mensagens,
      dadosUnidade: retorno?.dadosUnidadeWsVo,
...
module.exports = new SocUnidadeClient();
