...
/**
 * Cliente SOAP específico para a API SOC (CargoWs)
 * Segue o padrão do SocSetorClient/SocUnidadeClient para customizações de segurança e métodos.
 */
class SocCargoClient {
    constructor() {
        this.wsdl = config.socCargo.wsdl;
        this.username = config.socCargo.username;
        this.password = config.socCargo.password;
        this.client = null;
    }
    ...
    async createClient() {
        ...
        try {
            ...
            this.client = await soap.createClientAsync(this.wsdl, {
                wsdl_options: {
                    timeout: config.socCargo.timeout || 30000,
            ...
            throw new Error(`Erro ao criar cliente SOAP para SOC (Cargo): ${error.message}`);
        }
    }

    /**
     * Executa uma chamada SOAP para um método específico.
     * @param {string} method - Nome do método a ser chamado (ex: incluirCargo)
    ...
    async call(method, params) {
        ...
        try {
            ...
            if (!client[method]) {
                ...
                throw new Error(`Método ${method} não encontrado no WSDL do SOC (Cargo)`);
            }
            ...
            throw new Error(`Erro na chamada SOAP para ${method} (Cargo): ${error.message}`);
        }
    }
    ...
    formatResponse(response) {
        ...
        // Tenta encontrar o retorno em diferentes caminhos possíveis (adaptado para Cargo)
        const retorno = response?.CargoRetorno || response?.return || response;
        ...
        const resultado = {
            ...
            detalhes: mensagens,
            dadosCargo: retorno?.dadosCargoWsVo,
...
module.exports = new SocCargoClient();
