...
async function alterarHierarquia(hierarquiaData) {
  ...
  if (hierarquiaData.codigoEmpresa && !hierarquiaData.soc_codigo) {
    ...
    if (hierarquiaData.codigoEmpresa !== config.codEmpresa) {
      return {
        success: false,
        message: "A empresa pai deve ter um 'soc_codigo' antes de alterar Unidade/Setor/Cargo."
      };
    }
  }
  try {
    ...
    console.log(
      "Attempting to call alterarSituacao operation with args:",
      JSON.stringify(args, null, 2)
    );
    const [result] = await client.alterarSituacaoAsync(args);
    console.log("API Response:", JSON.stringify(result, null, 2));

    // A resposta da API SOC para alterar é um objeto que contém o resultado.
    // Baseado no JSON do usuário, o resultado está em HierarquiaRetorno.informacaoGeral
    const hierarquiaRetorno = result.AlterarHierarquiaResult || result.HierarquiaRetorno;
    ...
    if (informacaoGeral && informacaoGeral.numeroErros === 0) {
      return {
        ...
        message: "Hierarquia alterada com sucesso"
      };
    } else {
    ...
    return {
      success: false,
      error: error.message,
      message: "Erro ao alterar hierarquia"
    };
  }
}

// Função de teste (mantida para compatibilidade)
async function testAlterarHierarquia() {
  const testData = {
    codigoUnidade: "9",
    codigoSetor: "1",
    codigoCargo: "4",
  ...
  return await alterarHierarquia(testData);
}

module.exports = alterarHierarquia;

// Se executado diretamente, roda o teste
if (require.main === module) {
  testAlterarHierarquia();
}
